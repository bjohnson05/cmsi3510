...
DWORD WINAPI child( LPVOID passedData ) {
   ...
   int  timeToLive   = childData->millisecsToRun;       //   so we use the "->" to access fields

   // print that the thread has started
   printf( "%c has started successfully.\n", threadLetter );
...
int main( int argc, char * argv[] ) {
   ...
   for( i = 1; i < argc; i += 2 ) {
      ...
      data->millisecsToRun = atoi( argv[i + 1]);
      totalTime += data->millisecsToRun;
